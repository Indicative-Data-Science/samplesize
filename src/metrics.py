...
def generate_samples(data,height):
    ...
    for d,out_d in zip(int_d,int_d[1:]+[np.inf]):
        ...
        sample_size=[2**i for i in range(3,np.ceil(np.log2(max_size)).astype('int'))]
        insample_proportion=np.arange(0.1,1.1,0.1)
        reps=range(10)
        insample_size = (np.floor(p*s).astype('int') for s in sample_size for p in insample_proportion)
        outsample_size = (np.ceil((1-p)*s).astype('int') for s in sample_size for p in insample_proportion)
        for i,o in zip(insample_size,outsample_size):
            print(f'indicator {d},insample {i}, outsample {o}')
            for _ in reps:
                in_idx_sample=np.random.choice(in_idx,size=min(i,in_idx.shape[0]),replace=False)
                out_idx_sample=np.random.choice(out_idx,size=min(o,out_idx.shape[0]),replace=False)
...
def run_sample(data,sample_array,height):
    ...
    window_n=[10**i for i in range(2,6)]
    colnames= ["I_"+str(d) for d in int_d]+["W_"+str(n) for n in window_n]+['est_lb','est','est_ub']
    results=[]
    for i,sample in enumerate(sample_array.T):
        ...
        metrics=[int_metric(h,height,d) for d in int_d]+[window_metric(h,height,n) for n in window_n]
